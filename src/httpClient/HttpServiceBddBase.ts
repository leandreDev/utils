...
export class HttpServiceBddBase<T extends IBase> implements IHttpServiceBase<T> {
  constructor(
    conf: {
      bdd: Promise<Db>,
          ...
          return collection.findOneAndDelete(q)
            .then(result => {
              ...
              if (result.ok) {
                ...
                return new HttpResult<T>(new Error(result.lastErrorObject.message), this.debug, meta)
              }
            })
        })
      })
    ...
    return polonaisInverse(query, this.entity)
      .then((stackArr) => {
        ...
        return this.collection
          .then(collection => {
            const cursor: Cursor = collection.find(q, {projection:headers.$projection});
