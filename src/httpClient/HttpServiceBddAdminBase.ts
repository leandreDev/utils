...
export class HttpServiceBddAdminBase<T extends IBase>  implements IHttpServiceAdminBase<T> {
  constructor(
    conf: {
      bdd: Promise<Db>,
      ...
      debug: boolean,
      _class?: string,
      entity: { new(): Entity, cast(obj: any), check(target: any, isCompleteObj: boolean, path: string): string[] , castQueryParam(path: string, value: any): any}
    }
  ) {
  ...
  protected entity: { new(): Entity, cast(obj: any), check(target: any, isCompleteObj: boolean, path: string): string[] , castQueryParam(path: string, value: any): any};
