...
export class HttpServiceBddBaseView<T extends IBase> implements IHttpServiceBaseView<T> {
  constructor(
    conf: {
      bdd: Promise<Db>,
      ...
      debug: boolean,
      _class?: string,
      entity: { new(): Entity, cast(obj: any), check(target: any, isCompleteObj: boolean, path: string): string[], castQueryParam(path: string, value: any): any, getClassNameOfProp(path: string): string },
  ...
  public collections: { getHttpService(colName: string): HttpServiceBddBase<IBase> }
  protected entity: { new(): Entity, cast(obj: any), check(target: any, isCompleteObj: boolean, path: string): string[], castQueryParam(path: string, value: any): any, getClassNameOfProp(path: string): string };
  ...
  public get(
    ...
    return polonaisInverse(query, this.entity)
      .then((stackArr) => {
        ...
        // meta.offset = 0;
        // meta.pageSize = 1000;
        const pop: { propName: string, httpService: HttpServiceBddBase<IBase>, className: string }[] = [];
        while (stackArr.length > 0) {
          // de nouvelle operation
          let op: any = stackArr.shift();
          switch (op.name) {
            case "$pop":
              ...
              if (className) {
                const httpServ: HttpServiceBddBase<IBase> = this.collections.getHttpService(className);
                if (httpServ) {
        ...
        let CtxInt: CtxInterpretor = new CtxInterpretor(ctx);
        CtxInt.startPatern = "$ctx.";
        // let agg:any[] = CtxInt.updateArrEnv(filterAgg , true) ;
        let agg:any[] =  filterAgg.slice() ;
        if (this.debug) {
          meta.mongoquery =  agg ;
        } 
        return this.collection
          .then(collection => {
            const cursor: AggregationCursor = collection.aggregate(agg , {allowDiskUse:true});
                      ...
                      .then(() => {
                        ...
                        return popObj.httpService.collection
                          ...
                          .then((popArr) => {
                            ...
                            let lastPropName: string = pathArr.pop();
  ...
  public getValueOfPath(path: string[], obj: any, ids: ObjectId[], proxy: any[]) {
